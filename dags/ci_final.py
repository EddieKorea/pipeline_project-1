...
dag = DAG(
    ...
    end_date=datetime(2022, 4, 1, tzinfo=kst),
    catchup=False
)
    ...
    try:
        ...
        while True:
            ...
            for item in items:
                ...
                ciMinGong.append(item.select_one("code4").text)
                ciInOut.append(item.select_one("code21").text)
...
def ci_accident():
    ...
    try:
        ...
        sb.dbgout(f"ci_aciident FAIL! -> {str(ex)}")
    ...
    try:
        ...
        while True:
            ...
            for item in items:
                ...
                caInstLocal.append(item.select_one("caInstLocal").text if item.select_one("caInstLocal") else None)
                ...
                ciRegionName.append(item.select_one("name3").text if item.select_one("name3") else None)
                caInstCode.append(item.select_one("caInstCode").text if item.select_one("caInstCode") else None)
            if cnt == math.ceil(total/10000):
                break
            cnt += 1

        df = pd.DataFrame([caSeq, caInstNo, caInstLocal, caCuser, caCdate, ciInstCode, ciInst, ciSeq, ciRegionName, caInstCode]).T
        df.columns = ['caSeq', 'caInstNo', 'caInstLocal', 'caCuser', 'caCdate', 'ciInstCode', 'ciInst', 'ciSeq', 'ciRegionName', 'caInstCode']
        df['ciSeq'] = df['ciSeq'].astype('int')
...
[ciAll, ciAcci, ciInst, ciInspect, ciManage, cahCause, ciOper, ciPlace] >> toHDFS >> [ciAllSpark, ciAcciSpark, ciInstSpark, ciManageSpark, cahCauseSpark, ciFloorSpark, ciOperSpark, ciPlaceSpark]  >> slack
